...
const EditTrip = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('deleteImages', form.deleteImages);
    newImages.forEach((img) => formData.append('newImages', img));
    if (video) formData.append('video', video);

    try {
      await axios.put(`http://localhost:3000/trip/update-trip/${id}`, formData);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            type="file"
            multiple
            onChange={(e) => setNewImages((prev) => [...prev, ...e.target.files])}
